...
object BankAccountMain extends App {
    ...
    b.deposit(100);  println(b)
    
    b.withdraw(100);  println(b)
    
    println("SavingAccount")
    
    val a = new SavingsAccount(100.0, 0.10, 2, 2.0)
    
    a.deposit(100); println(a)
    
    a.withdraw(100); println(a)
    
    a.deposit(100); println(a)
    
    a.withdraw(100); println(a)
    
    a.earnMonthlyInterest; println("Monthly Interest: " + a)
...
class SavingsAccount(	initialBalance: Double,	val interestRateYear: Double = 0.10,	
       val freeTransactions: Int = 3,	val comission: Double = 1.0) extends BankAccount(initialBalance) {
